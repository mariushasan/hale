...
function AssaultRifle.animateBullet(hitPosition, direction, animationStartOffset)    
    local bulletStart = visualRig:FindFirstChild("BulletStart", true)
    local startPosition = bulletStart and bulletStart.WorldPosition
    startPosition = startPosition + animationStartOffset
    
    local endPosition = startPosition + (direction.Unit * AssaultRifleConstants.MAX_BULLET_DISTANCE)
    ...
    return function()
        ...
        local newPosition = startPosition + (direction * distance)
...
function AssaultRifle.handleFireFromClient()
    ...
    local fireRate = AssaultRifleConstants.FIRE_COOLDOWN
    ...
    for _, bullet in ipairs(bullets) do
        ...
        if bullet.id and bullet.id:find("^" .. player.Name .. "_") then
            ...
            for _, raycastData in ipairs(bullet.raycastData) do                
                ...
                local raycastDistance = AssaultRifleConstants.MAX_BULLET_DISTANCE -- Max shooting distance
            ...
            local updateFunction = AssaultRifle.animateBullet(hitPosition, bullet.animationDirection, bullet.animationStartOffset)
...
function AssaultRifle.handleFireFromServer(bullets)
    ...
    for _, bullet in ipairs(bullets) do
        local updateFunction = AssaultRifle.animateBullet(bullet.hitPosition, bullet.animationDirection, bullet.animationStartOffset)
        bulletAnimations[bullet.id] = {
            update = updateFunction,
        }
    end
